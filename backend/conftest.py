...
def django_db_setup(django_db_blocker):
    ...
    with django_db_blocker.unblock():
        ...
        with schema_context("public"):
            ...
            settings.MIDDLEWARE = [
                m for m in settings.MIDDLEWARE if not m.startswith("django_tenants")
            ]
...
def tenant(django_db_setup, django_db_blocker):
    ...
    def _create_tenant():
        ...
        try:
            ...
            with connection.cursor() as cursor:
                cursor.execute(
                    "SELECT pg_catalog.setval(pg_get_serial_sequence('core_school', 'id'), 1, false);"
                )
                cursor.execute(
                    "SELECT pg_catalog.setval(pg_get_serial_sequence('core_domain', 'id'), 1, false);"
                )
                cursor.execute(
                    "SELECT pg_catalog.setval(pg_get_serial_sequence('accounts_user', 'id'), 1, false);"
                )
            ...
            with schema_context(get_public_schema_name()):
                ...
                Domain.objects.create(
                    domain=f"{schema_name}.localhost", tenant=school, is_primary=True
                )
        ...
        def generic(self, method, path, *args, **kwargs):
            ...
            try:
                ...
                with schema_context(self.tenant.schema_name):
                    ...
                    self.logger.debug(
                        f"Current schema before request: {current_schema}"
                    )
                    assert (
                        current_schema == self.tenant.schema_name
                    ...
                    self.logger.debug(
                        f"Request to {path} completed with status {response.status_code}"
                    )
                ...
                try:
                    with transaction.atomic():
                        with schema_context("public"):
                            ...
                            logger.info(
                                f"Found {schools.count()} test schools to clean up"
                            )
                            ...
                            logger.info(
                                "Successfully cleaned up test schemas and domains"
                            )
                            break

                except Exception as e:
                    logger.error(
                        f"Error during cleanup (attempt {retry_count + 1}): {e}",
                        exc_info=True,
                    )
                    retry_count += 1
