...
class School(TenantMixin):
    ...
    def save(self, *args, **kwargs):
        ...
        if self.student_strength > 500:
            logger.error(
                f"School {self.name} exceeded free tier student limit: {self.student_strength}"
            )
            raise FreeTierLimitExceeded("Free tier allows maximum of 500 students")

        if self.staff_count > 50:
            logger.error(
                f"School {self.name} exceeded free tier staff limit: {self.staff_count}"
            )
        ...
        try:
            ...
            if creating and self.is_approved:
                ...
                try:
                    ...
                    send_mail(
                        ...
                        html_message=render_to_string(
                            "emails/school_approved.html", context
                        ),
                    )
                    logger.info(
                        f"Sent welcome email to school admin: {self.principal_email}"
                    )

                except User.DoesNotExist as e:
                    logger.error(
                        f"Failed to create admin user for school {self.name}: {str(e)}"
                    )
                    raise SchoolApprovalError(f"Failed to create admin user: {str(e)}")
                except Exception as e:
                    logger.error(
                        f"Error in school approval process for {self.name}: {str(e)}"
                    )
                    raise SchoolApprovalError(
                        f"School approval process failed: {str(e)}"
                    )
                ...
                try:
                    ...
                    logger.error(
                        f"Failed to send welcome email to {self.principal_email}: {str(e)}"
                    )
