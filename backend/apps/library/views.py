    ...
    def get_queryset(self):
        ...
        with schema_context(tenant.schema_name):
            ...
            with transaction.atomic():
                try:
                    queryset = Book.objects.select_related('school').filter(school=tenant)
        ...
        with schema_context(tenant.schema_name):
            ...
            with transaction.atomic():
                try:
                    ...
                    logger.info(f"Created book {instance.pk} in schema {tenant.schema_name}")
                    return instance
                except Exception as e:
            ...
            with transaction.atomic():
                try:
                    ...
                    book_issue = BookIssue.objects.create(
                        ...
                        due_date=request.data.get('due_date', timezone.now().date() + timezone.timedelta(days=14))
                    )
                    ...
                    logger.info(f"Book {book.id} issued to student {student.id} in schema {tenant.schema_name}")
                    return Response(BookIssueSerializer(book_issue).data, status=status.HTTP_201_CREATED)
...
class BookIssueViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        ...
        with schema_context(tenant.schema_name):
            ...
            with transaction.atomic():
                try:
                    queryset = BookIssue.objects.select_related('book', 'student', 'school').filter(school=tenant)
                    count = queryset.count()
                    logger.info(f"Found {count} book issues in schema {tenant.schema_name}")
                    return queryset.all()
                except Exception as e:
        ...
        with schema_context(tenant.schema_name):
            ...
            with transaction.atomic():
                try:
                    ...
                    book_issue = BookIssue.objects.select_related('book').get(pk=pk, school=tenant)
                    ...
                    logger.info(f"Book {book.id} returned successfully in schema {tenant.schema_name}")
                    return Response({'message': 'Book returned successfully'}, status=status.HTTP_200_OK)
                    
                except BookIssue.DoesNotExist:
                    logger.error(f"Book issue {pk} not found in schema {tenant.schema_name}")
                    return Response(
