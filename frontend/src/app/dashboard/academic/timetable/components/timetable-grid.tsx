        ...
        <Card key={day}>
          ...
          <CardContent>
            <div className="space-y-4">
              {entries?.map(entry => (
                <div key={entry.id} className="p-3 bg-secondary/50 rounded-lg space-y-2">
                  <div className="flex justify-between items-start">
                    ...
                    {isAdmin && <EditTimetableEntryDialog entry={entry} sectionId={sectionId} />}
                  </div>
                  <p className="text-sm">
                    {entry.subject.teacher.first_name} {entry.subject.teacher.last_name}
                  </p>
                </div>
              ))}
              {!entries?.length && (
                <p className="text-sm text-gray-500 text-center py-2">No classes scheduled</p>
              )}
            </div>
          </CardContent>
        </Card>
