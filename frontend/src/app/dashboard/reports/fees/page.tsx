...
export default function FeeReportsPage() {
  ...
  const { data: feeData } = useQuery({
    queryKey: ['fee-report', selectedClass, selectedSection, selectedStatus, dateRange],
    queryFn: () =>
      reportsService.getFeeReport({
        ...
        payment_status:
          selectedStatus !== 'all' ? (selectedStatus as 'paid' | 'pending' | 'overdue') : undefined,
        ...
        (feeData?.reduce(
          (acc, student) =>
            ...
            student.payment_history.reduce(
              (sum, payment) => (payment.status === 'overdue' ? sum + payment.amount : sum),
              0
            ),
          0
        ) || 0),
  ...
  return (
    <div className="container mx-auto py-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
        ...
        <Card className="p-6">
          <h2 className="text-lg font-semibold mb-4">Payment Collection Trend</h2>
          <div className="h-[300px]">
            <ResponsiveContainer width="100%" height="100%">
              <LineChart data={paymentTrendData}>
                ...
                <Tooltip />
                <Legend />
                <Line type="monotone" dataKey="amount" name="Collection Amount" stroke="#3b82f6" />
              </LineChart>
            </ResponsiveContainer>
          </div>
        </Card>
      ...
      {/* Detailed Table */}
      {feeData && <FeeDetails data={feeData} />}
    </div>
  );
}
