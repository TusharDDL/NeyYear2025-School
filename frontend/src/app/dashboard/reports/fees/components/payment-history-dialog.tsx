...
export function PaymentHistoryDialog({ student, onClose }: PaymentHistoryDialogProps) {
  ...
  return (
    <Dialog open onOpenChange={onClose}>
      <DialogContent className="max-w-4xl">
        <DialogHeader>
          <DialogTitle>Payment History - {student.student_name}</DialogTitle>
        </DialogHeader>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            <Card className="p-4">
              ...
              <div className="mt-1 flex items-baseline">
                ...
                <p className="text-2xl font-bold">{student.total_fees.toLocaleString('en-IN')}</p>
              </div>
            </Card>
              ...
              <Table>
                ...
                <TableBody>
                  {student.payment_history.map((payment, index) => (
                    <TableRow key={index}>
                      <TableCell>{format(new Date(payment.date), 'MMM d, yyyy h:mm a')}</TableCell>
                      <TableCell>
