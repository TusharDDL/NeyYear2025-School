    ...
    await waitFor(
      () => {
        ...
        const searchCall = calls.find(call => {
          ...
          return (
            url.pathname === '/api/books/search' && url.searchParams.get('query') === 'physics'
          );
        });
  ...
  it('renders circulation records', async () => {
    ...
    await waitFor(
      () => {
        ...
        const fetchUrl = mockFetch.mock.calls[0][0];
        expect(
          typeof fetchUrl === 'string' && fetchUrl.includes('/api/books/circulation')
        ).toBeTruthy();
      },
      { timeout: 5000 }
    );
    ...
    await waitFor(
      () => {
        ...
        const returnCall = calls.find(
          call => call[0] === '/api/books/circulation/1/return' && call[1]?.method === 'POST'
        );
